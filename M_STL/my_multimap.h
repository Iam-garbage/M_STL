...
template<class Key, class T, class Compare = std::less<Key>,
    ...
    public:
        ...
        explicit multimap(const allocator_type& alloc) :tree(alloc) {}
        ...
        multimap(value_type* first, value_type* last,
            const key_compare& comp = key_compare(),
            const allocator_type& alloc = allocator_type()) :tree(comp, alloc) {
            tree.insert_equal(first, last);
        }
        multimap(const multimap& x) :tree(x.tree) {}
        multimap(multimap&& x) :tree(std::move(x.tree)) {}
        multimap(std::initializer_list<value_type> il,
            const key_compare& comp = key_compare(),
            const allocator_type& alloc = allocator_type()) :tree(comp, alloc) {
            tree.insert_equal(il);
        }
        //~map() { ~tree(); }
    ...
    public:
        ...
        data_type& operator[] (const key_type& k) {
            ...
            if (x == end() || key_comp()(k, (*x).first)) {
                x = tree.insert_equal(x, value_type(k, data_type()));
            }
            return (*x).second;
        }
